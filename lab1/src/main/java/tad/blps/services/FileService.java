...
public class FileService {
    ...
    public List<File> getFilesByUserId(Long userId) {
        Optional<List<File>> fileList = fileRepository.findByUserIdOrderBySizeAsc(userId);
        return fileList.orElse(null);
    }

    public List<File> getFilesByUserIdAndSpecificSorting(Long userId, String param, String order) {
        List<File> fileList;
        if(order.equals("DESC")) {
            fileList = fileRepository.findAll(Sort.by(Sort.Direction.DESC, param));
        } else {
            fileList = fileRepository.findAll(Sort.by(Sort.Direction.ASC, param));
        }
        return fileList.stream().filter(c -> c.getUserId()== userId).toList();

    }
    
    public List<File> getFilesByFilter(Long userId, String param, String order, String filterBy, String value) {
        List<File> res = getFilesByUserIdAndSpecificSorting(userId, param, order);
        switch(filterBy) {
            case "download_url" -> {
                return res.stream().filter(c->c.getDownload_url().equals(value)).toList();
            }
            case "filename_source" -> {
                return res.stream().filter(c->c.getFilename_source().equals(value)).toList();
            }
            case "idstr" -> {
                return res.stream().filter(c->c.getIdstr().equals(value)).toList();
            }
            case "None" -> {
                return res;
            }
            case "download_cnt" -> {
                return res.stream().filter(c->c.getDownload_cnt() == Integer.parseInt(value)).toList();
            }
            case "size" -> {
                return res.stream().filter(c -> c.getSize() ==  Integer.parseInt(value)).toList();
            }
            default -> {
                return null;
            }
...
//    return status of file
    public void uploadFile(FileDTO file, Long userId)
        ...
        if(fileList.isPresent()) {
            Long totalSize = fileList.get().stream().mapToLong(File::getSize).sum();
    ...
    public String generateIdString(String name) throws NoSuchAlgorithmException {
        String ret = "";
